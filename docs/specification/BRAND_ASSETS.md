...
`brand/` is the canonical, versioned source of the osTRIS identity. The artwork is derived from the current book-cover source: dark teal `#173F3B`, ivory `#F4F0E5`, amber `#D59B3D`, jade `#2D5B57`, ink `#15201F`, and network teal `#9CB6B1`.
...
osTRIS modules own and expose these assets through the module extension contract. Generic IDAX Shell UI must not hardcode them.
